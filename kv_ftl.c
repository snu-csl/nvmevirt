...
unsigned int new_mapping_entry_by_key(struct kv_ftl *kv_ftl, unsigned char *key, int key_len, int val_len, size_t val_offset) {
	unsigned int slot = -1;
	unsigned int prev_slot;
	BUG_ON(val_offset < 0 || val_offset >= vdev->config.storage_size);
...
unsigned int update_mapping_entry(struct kv_ftl *kv_ftl, struct nvme_kv_command cmd) {
    ...
    if (found) {
		NVMEV_DEBUG("Updating mapping length %llu to %u for key %s\n",
               kv_ftl->kv_mapping_table[slot].length, cmd_value_length(cmd), cmd.kv_store.key);
        kv_ftl->kv_mapping_table[slot].length = cmd_value_length(cmd);
    }
...
static unsigned int __do_perform_kv_batched_io(struct kv_ftl *kv_ftl, int opcode, char * key, int key_len, char * value, int val_len) {
	size_t offset;
	size_t new_offset = 0;
	struct mapping_entry entry;
	int is_insert = 0;
...
static unsigned int __do_perform_kv_batch(struct kv_ftl *kv_ftl, struct nvme_kv_command cmd, unsigned int * status) {
	...
	for (i = 0; i < sub_cmd_cnt; i++) {
		...
		NVMEV_DEBUG("sub-payload %d %d %d %d %s %s", payload->batch_head.attr[i].opcode, key_len, val_len, sub_len, key, value);
...
bool kv_proc_nvme_io_cmd(struct nvmev_ns *ns, struct nvmev_request *req, struct nvmev_result *ret)
{
	...
	case nvme_cmd_read:
		ret->nsecs_target = __schedule_io_units(
				cmd->common.opcode, cmd->rw.slba, __cmd_io_size((struct nvme_rw_command*)cmd), __get_wallclock());
		break;
	case nvme_cmd_flush:
	...
	case nvme_cmd_kv_batch:
		ret->nsecs_target = __schedule_io_units(
				cmd->common.opcode, 0, cmd_value_length(*((struct nvme_kv_command *)cmd)), __get_wallclock());
		NVMEV_INFO("%d, %llu, %llu\n", cmd_value_length(*((struct nvme_kv_command *)cmd)), __get_wallclock(), ret->nsecs_target);
		break;
